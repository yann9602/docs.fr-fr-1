...
Vous avez tenté de convertir un type en un autre type qui n’est peut-être pas en mesure de contenir la valeur, comme un `Long` à une `Integer`, tandis que le commutateur de vérification de type ([Option Strict, instruction](../../visual-basic/language-reference/statements/option-strict-statement.md)) est défini sur `On`.  
...
1.  Déterminez si une conversion de n’importe quel type existe de `<type1>` à `<type2>`. Si les deux sont [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] des types élémentaires, ou si les deux sont des instances de classes, vous pouvez généralement effectuer cette détermination en consultant la table de [Conversions étendues et restrictives](../../visual-basic/programming-guide/language-features/data-types/widening-and-narrowing-conversions.md).  
