...
Une expression est évaluée comme étant un type qui ne peut pas être converti \(« boxed »\) par le common language runtime \(CLR\).  
 ...
 Si vous n’avez pas utilisé `CType` ou `CObj` dans l’instruction contenant cette expression, [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] a tenté une conversion implicite qui provoque cette erreur.  
