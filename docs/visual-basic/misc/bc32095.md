...
'\(' inattendu. Les tableaux de types génériques non instanciés ne sont pas autorisés.  
 ...
 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] ne peut pas compiler un tableau s’il ne possède pas un type de données spécifique. Vous ne pouvez pas utiliser un paramètre de type de données d’un type générique comme type de données d’un tableau.  
