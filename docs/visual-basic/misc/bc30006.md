...
Une référence au module '\<`modulename`\>' contenant la définition pour l’événement '\<`eventname`\>' est requise. Ajoutez\-en une à votre projet.  
 ...
 L’événement est défini dans un module qui n’est pas directement référencé dans votre projet. Le compilateur [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] nécessite une référence pour éviter toute ambiguïté au cas où l’événement serait défini dans plusieurs modules.  
