...
ms.topic: "article"
helpviewer_keywords: 
  - "méthodes génériques, inférence de type"
  ...
  - "définitions de type générique"
  - "classes génériques (.NET Framework)"
  - "génériques (.NET Framework), interfaces"
  - "génériques (.NET Framework), à propos"
  - "génériques (.NET Framework)"
  - "collections génériques (.NET Framework)"
  - "délégués génériques (.NET Framework)"
  - "arguments de type générique"
  - "génériques (.NET Framework), délégués"
  - "génériques (.NET Framework), fonctionnalités"
  - "contraintes (.NET Framework)"
  - "types génériques"
...
-   Les énumérations ne peuvent pas avoir de paramètres de type générique. Une énumération ne peut être générique que de manière secondaire \(par exemple, parce qu'elle est imbriquée dans un type générique qui est défini à l'aide de Visual Basic, C\# ou C\+\+\). Pour plus d’informations, consultez la section "Énumérations" dans [Système de type commun](../../../docs/standard/base-types/common-type-system.md).  
...
-   L'espace de noms <xref:System.Collections.ObjectModel> catalogue d'autres types de collections génériques, tels que la classe générique <xref:System.Collections.ObjectModel.ReadOnlyCollection%601>, qui sont utiles pour exposer des modèles objet aux utilisateurs de vos classes.  
 ...
 Visual C\+\+, C\# et Visual Basic fournissent une prise en charge complète de la définition et de l'utilisation des génériques. Pour plus d’informations sur la prise en charge des langages, consultez [Types génériques Visual Basic](../Topic/Generic%20Types%20in%20Visual%20Basic%20\(Visual%20Basic\).md), [Introduction aux génériques](../Topic/Introduction%20to%20Generics%20\(C%23%20Programming%20Guide\).md) et [Overview of Generics in Visual C\+\+](../Topic/Overview%20of%20Generics%20in%20Visual%20C++.md).  
...
|[Comment : définir un type générique avec l'émission de réflexion](../../../docs/framework/reflection-and-codedom/how-to-define-a-generic-type-with-reflection-emit.md)|Explique comment générer des assemblys dynamiques qui incluent des types et des méthodes génériques.|  
|[Types génériques Visual Basic](../Topic/Generic%20Types%20in%20Visual%20Basic%20\(Visual%20Basic\).md)|Explique la fonction des génériques pour les utilisateurs de Visual Basic et fournit des procédures concernant l'utilisation et la définition des types génériques.|  
|[Introduction aux génériques](../Topic/Introduction%20to%20Generics%20\(C%23%20Programming%20Guide\).md)|Fournit une vue d'ensemble de la définition et de l'utilisation des types génériques pour les utilisateurs de C\#.|  
|[Overview of Generics in Visual C\+\+](../Topic/Overview%20of%20Generics%20in%20Visual%20C++.md)|Explique la fonction des génériques pour les utilisateurs C\+\+, y compris les différences entre les génériques et les modèles.|  
