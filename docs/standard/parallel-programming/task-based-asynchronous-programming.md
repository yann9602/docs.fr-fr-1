...
>  Notez que la culture du thread appelant dans le cadre du contexte d’une tâche s’applique aux applications qui *ciblent* le [!INCLUDE[net_v46](../../../includes/net-v46-md.md)], et non celles qui *s’exécutent sous* le [!INCLUDE[net_v46](../../../includes/net-v46-md.md)]. Vous pouvez cibler une version particulière du .NET Framework lorsque vous créez votre projet dans Visual Studio en sélectionnant une version dans la liste déroulante en haut de la **nouveau projet** boîte de dialogue, ou en dehors de Visual Studio, vous pouvez utiliser la <xref:System.Runtime.Versioning.TargetFrameworkAttribute> attribut. Pour les applications qui ciblent des versions du .NET Framework antérieures au [!INCLUDE[net_v46](../../../includes/net-v46-md.md)], ou qui ne ciblent pas une version spécifique du .NET Framework, la culture d'une tâche demeure déterminée par la culture du thread sur lequel la tâche s'exécute.  
 ...
 Si vous utilisez Visual Studio, vous pouvez omettre le <xref:System.Runtime.Versioning.TargetFrameworkAttribute> d’attribut et les sélectionner à la place le .NET Framework 4.6 comme cible, lorsque vous créez le projet dans le **nouveau projet** boîte de dialogue.  
...
## <a name="creating-task-continuations"></a>Création de continuations de tâches  
 Le <xref:System.Threading.Tasks.Task.ContinueWith%2A?displayProperty=nameWithType> et <xref:System.Threading.Tasks.Task%601.ContinueWith%2A?displayProperty=nameWithType> méthodes vous permettent de spécifier une tâche à démarrer lorsque le *antécédent* se termine. Une référence à la tâche antécédente est passée au délégué de la tâche de continuation afin qu'il puisse examiner l'état de la tâche antécédente et, en récupérant la valeur de la propriété <xref:System.Threading.Tasks.Task%601.Result%2A?displayProperty=nameWithType>, utiliser la sortie de l'antécédent comme entrée pour la continuation.  
...
## <a name="waiting-for-tasks-to-finish"></a>Attente de fin d’exécution de tâches  
 Le <xref:System.Threading.Tasks.Task?displayProperty=nameWithType> et <xref:System.Threading.Tasks.Task%601?displayProperty=nameWithType> types fournissent plusieurs surcharges de la <xref:System.Threading.Tasks.Task.Wait%2A?displayProperty=nameWithType> et <!--zz <xref:System.Threading.Tasks.Task%601.Wait%2A?displayProperty=nameWithType>--> `System.Threading.Tasks.Task.Wait` méthodes qui vous permettent d’attendre une tâche se termine. De plus, les surcharges des méthodes statiques <xref:System.Threading.Tasks.Task.WaitAll%2A?displayProperty=nameWithType> et <xref:System.Threading.Tasks.Task.WaitAny%2A?displayProperty=nameWithType> vous permettent d'attendre que certaines ou toutes les tâches soient terminées.  
...
## <a name="canceling-tasks"></a>Annulation des tâches  
 La classe `Task` prend en charge l'annulation coopérative et s'intègre pleinement aux classes <xref:System.Threading.CancellationTokenSource?displayProperty=nameWithType> et <xref:System.Threading.CancellationToken?displayProperty=nameWithType>, qui ont été introduites dans .NET Framework 4. De nombreux constructeurs de la classe <xref:System.Threading.Tasks.Task?displayProperty=nameWithType> prennent un objet <xref:System.Threading.CancellationToken> en tant que paramètre d'entrée. La plupart des surcharges <xref:System.Threading.Tasks.TaskFactory.StartNew%2A> et <xref:System.Threading.Tasks.Task.Run%2A> incluent également un paramètre <xref:System.Threading.CancellationToken>.  
...
-   Lorsque vous créez des tâches de continuation à partir de plusieurs antécédents, utilisez la <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAll%2A> méthode ou <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAny%2A> ou leurs équivalents dans la <xref:System.Threading.Tasks.Task%601> classe. Pour plus d’informations, consultez [Chaînage des tâches à l’aide de tâches de continuation](../../../docs/standard/parallel-programming/chaining-tasks-by-using-continuation-tasks.md).  
