---
title: "Exécution d’opérations de mise en forme"
description: "Exécution d’opérations de mise en forme"
keywords: .NET, .NET Core
author: stevehoag
manager: wpickett
ms.date: 07/25/2016
ms.topic: article
ms.prod: .net-core
ms.technology: .net-core-technologies
ms.devlang: dotnet
ms.assetid: 982f0b64-7e69-4bbe-a688-0117f86a603c
translationtype: Human Translation
ms.sourcegitcommit: fb00da6505c9edb6a49d2003ae9bcb8e74c11d6c
ms.openlocfilehash: 7cc5f4e68cb944a2684c58af866afe1edd66d43b

---

# <a name="performing-formatting-operations"></a>Exécution d’opérations de mise en forme

Les rubriques suivantes fournissent des instructions pas à pas pour effectuer des opérations de mise en forme spécifiques.

* [Guide pratique pour remplir un nombre avec des zéros non significatifs](pad-number.md)

* [Guide pratique pour définir et utiliser des fournisseurs de format numérique personnalisés](define-custom.md)

* [Guide pratique pour extraire le jour de la semaine d’une date spécifique](extract-day.md)

* [Guide pratique pour effectuer un aller-retour de valeurs de date et d’heure](roundtrip.md)

* [Guide pratique pour afficher les millisecondes dans les valeurs de date et d’heure](display-milliseconds.md)

* [Guide pratique pour afficher des dates dans des calendriers non grégoriens](display-dates.md)

## <a name="see-also"></a>Voir aussi

[Mise en forme des types](formatting-types.md)



<!--HONumber=Nov16_HO3-->


