...
Cet exemple montre comment ajouter, récupérer, mettre à jour et supprimer des éléments dans un [System.Collections.Concurrent.ConcurrentDictionary&lt;TKey, TValue&gt;](https://docs.microsoft.com/dotnet/core/api/System.Collections.Concurrent.ConcurrentDictionary-2). Cette classe de collection est une implémentation thread-safe. Nous vous recommandons de l’utiliser chaque fois que plusieurs threads tentent d’accéder aux éléments de façon simultanée.
...
```
[ConcurrentDictionary&lt;TKey, TValue&gt;](https://docs.microsoft.com/dotnet/core/api/System.Collections.Concurrent.ConcurrentDictionary-2) est conçu pour les scénarios multithreads. Vous n’avez pas besoin d’utiliser des verrous dans votre code pour ajouter ou supprimer des éléments dans la collection. Toutefois, il est toujours possible pour un thread de récupérer une valeur et pour un autre thread de mettre à jour immédiatement la collection en donnant une nouvelle valeur à la même clé.

En outre, bien que toutes les méthodes de `ConcurrentDictionary<TKey, TValue>` soient thread-safe, toutes les méthodes sont atomiques, en particulier `GetOrAdd` et `AddOrUpdate`. Le délégué utilisateur passé à ces méthodes est appelé en dehors du verrou interne du dictionnaire. (Cela permet d’empêcher un code inconnu de bloquer tous les threads.) Par conséquent, la séquence d’événements suivante peut se produire :

1. Le threadA appelle `GetOrAdd`, ne trouve pas d’élément et crée un nouvel élément à ajouter en appelant le délégué valueFactory.

2. Simultanément, le threadB appelle `GetOrAdd`, son délégué valueFactory est appelé et atteint le verrou interne avant le threadA. Sa nouvelle paire clé-valeur est alors ajoutée au dictionnaire.
...
Par conséquent, il n’est pas garanti que les données retournées par `GetOrAdd` sont les mêmes que celles créées par le valueFactory du thread. Une séquence d’événements similaire peut se produire lors de l’appel de la méthode `AddOrUpdate`. 
