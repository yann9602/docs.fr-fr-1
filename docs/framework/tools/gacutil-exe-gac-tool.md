...
```  
 ...
 La commande suivante installe `myDll.dll` dans le Global Assembly Cache et ajoute une référence pour le décompter.  Le paramètre scheme, `FILEPATH`, et le paramètre id, `c:\applications\myApp\myApp.exe` spécifient le chemin d'accès à l'application qui installe `myDll.dll.`. Le paramètre description est spécifié en tant que `MyApp`.  
