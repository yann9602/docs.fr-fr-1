...
-   Certaines causes d'invalidation d'images ont été éliminées.  
 ...
 Sous Windows 8, consultez [Native Image Task](http://msdn.microsoft.com/fr-fr/9b1f7590-4e0d-4737-90ef-eaf696932afb).  
  
 Pour plus d'informations sur l'utilisation de Ngen.exe et du service d'images natives, consultez [Native Image Service](http://msdn.microsoft.com/fr-fr/b15e0e32-59cb-4ae4-967c-6c9527781309).  
  
> [!NOTE]
>  Vous trouverez la syntaxe de Ngen.exe pour les versions 1.0 et 1.1 du .NET Framework dans [Native Image Generator \(Ngen.exe\) Legacy Syntax](http://msdn.microsoft.com/fr-fr/5a69fc7a-103f-4afc-8ab4-606adcb46324).  
...
## Actions  
 Le tableau ci\-après décrit la syntaxe de chaque `action`.  Pour consulter une description de chaque partie d'une `action`, consultez les tableaux [Arguments](#ArgumentTable), [Niveaux de priorité](#PriorityTable), [Scénarios](#ScenarioTable) et [Config](#ConfigTable).  Le tableau [Options](#OptionTable) décrit les `options` et les commutateurs d'aide.  
...
|`3`|Les images natives sont installées lorsque le service d'images natives détecte que l'ordinateur est inactif.  Consultez [Native Image Service](http://msdn.microsoft.com/fr-fr/b15e0e32-59cb-4ae4-967c-6c9527781309).|  
...
|`/verbose`|Afficher des informations détaillées sur le débogage. **Note:**  En raison des limitations du système d'exploitation, cette option n'affiche pas autant d'informations supplémentaires sous Windows 98 et sous Windows Millennium.|  
...
>  N'exécutez pas Ngen.exe sur les assemblys dont le niveau de confiance n'est pas suffisant.  Si vous démarrez avec le [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)], Ngen.exe compile des assemblys avec une confiance totale et la stratégie de sécurité d'accès du code \(CAS\) n'est plus évaluée.  
 ...
 Ngen.exe génère des images natives pour l'assembly spécifié et toutes ses dépendances.  Les dépendances sont déterminées à partir de références du manifeste d'assembly.  Le seul scénario dans lequel vous devez installer une dépendance séparément est lorsque l'application la charge à l'aide de la réflexion, en appelant la méthode <xref:System.Reflection.Assembly.Load%2A?displayProperty=fullName> par exemple.  
...
### Démarrage d'applications plus rapide  
...
### Importance des adresses de base d'assemblys  
...
### Spécification d'une indication sur la liaison d'une dépendance  
 Appliquez <xref:System.Runtime.CompilerServices.DependencyAttribute> à un assembly pour indiquer la probabilité de chargement d'une dépendance spécifiée.  <xref:System.Runtime.CompilerServices.LoadHint?displayProperty=fullName> indique que la liaison matérielle est appropriée, <xref:System.Runtime.CompilerServices.LoadHint> indique que la valeur par défaut de la dépendance doit être utilisée et <xref:System.Runtime.CompilerServices.LoadHint> indique que la liaison matérielle n'est pas appropriée.  
...
-   Version du système d'exploitation, si la modification se fait de la famille Windows 9x vers la famille Windows NT.  
...
```  
 ...
 Pour obtenir des exemples liés au service d'images natives, consultez [Native Image Service](http://msdn.microsoft.com/fr-fr/b15e0e32-59cb-4ae4-967c-6c9527781309).  
  
## Tâche d'image native  
 La tâche d'image native est une tâche Windows qui génère et conserve des images natives.  La tâche d'image native génère et libère les images natives automatiquement pour les scénarios pris en charge.  \(Consultez [Creating Native Images](http://msdn.microsoft.com/fr-fr/2bc8b678-dd8d-4742-ad82-319e9bf52418).\) Elle permet aussi aux programmes d'installation d'utiliser [Ngen.exe \(Générateur d'images natives\)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) pour créer et mettre à jour des images natives à un moment différé.  
...
|NET Framework NGEN v4.0.30319 64|Non|Oui|  
 ...
 La tâche d'image native est disponible dans le .NET Framework 4.5 et versions ultérieures, dans le cadre d'une exécution sur Windows 8 ou version ultérieure.  Dans les versions antérieures de Windows, le .NET Framework utilise le [Native Image Service](http://msdn.microsoft.com/fr-fr/b15e0e32-59cb-4ae4-967c-6c9527781309).  
...
> [!NOTE]
>  Sur Windows Vista, le nom affiché pour le service d'images natives est « Microsoft.NET Framework NGEN v2.0.50727\_X86 » ou « Microsoft.NET Framework NGEN v2.0.50727\_X64 ».  Sur toutes les versions antérieures de Microsoft Windows, le nom est « .NET Runtime Optimization Service v2.0.50727\_X86 » ou « .NET Runtime Optimization Service v2.0.50727\_X64 ».  
...
## Voir aussi  
 [Native Image Service](http://msdn.microsoft.com/fr-fr/b15e0e32-59cb-4ae4-967c-6c9527781309)   
 [Native Image Task](http://msdn.microsoft.com/fr-fr/9b1f7590-4e0d-4737-90ef-eaf696932afb)   
