---
title: "Point de terminaison de m&#233;tadonn&#233;es s&#233;curis&#233; personnalis&#233;e | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
3.  Si le client et le service ne parviennent pas à communiquer, consultez la rubrique [Troubleshooting Tips](http://msdn.microsoft.com/fr-fr/8787c877-5e96-42da-8214-fa737a38f10b).  
...
1.  Sur le serveur, exécutez `setup.bat service`.L'exécution de `setup.bat` à l'aide de l'argument `service` crée un certificat de service portant le nom de domaine complet de l'ordinateur, puis exporte ce certificat vers un fichier appelé Service.cer.  
  
2.  Sur le serveur, modifiez Web.config en fonction du nouveau nom de ce certificat.En d'autres termes, remplacez l'attribut `findValue` de l'élément [\<serviceCertificate\>](../../../../docs/framework/configure-apps/file-schema/wcf/servicecertificate-of-clientcredentials-element.md) par le nom de domaine complet de l'ordinateur.  
  
3.  Copiez le fichier Service.cer du répertoire de service dans le répertoire client sur l'ordinateur client.  
  
4.  Sur le client, exécutez `setup.bat client`.L'exécution de `setup.bat`  à l'aide de l'argument `client`crée un certificat client appelé client.com, puis exporte ce certificat vers un fichier appelé Client.cer.  
...
>   
>  `<LecteurInstall>:\WF_WCF_Samples\WCF\Extensibility\Metadata\CustomMexEndpoint`  
