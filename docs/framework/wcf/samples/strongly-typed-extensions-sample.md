...
```  
 ...
 Le `<in-reply-to>` élément spécifie trois attributs requis (`ref`, `type` et `href`) en tenant également compte de la présence d’attributs d’extension supplémentaires et des éléments d’extension.  
