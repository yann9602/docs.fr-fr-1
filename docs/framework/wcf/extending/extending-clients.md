---
title: "Extension de clients | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Clients  
 Sur un client, un canal ou objet client [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] convertit des appels de méthode en messages sortants et messages entrants aux résultats de l'opération retournés à l'application appelante.\(Pour plus d'informations sur les types de clients, consultez [Architecture du client WCF](../../../../docs/framework/wcf/feature-details/client-architecture.md).\)  
...
-   Validation des messages personnalisés.Un utilisateur souhaite s'assurer qu'un message est valide pour un schéma spécifique.Cela peut être accompli en implémentant l'interface <xref:System.ServiceModel.Dispatcher.IClientMessageInspector> et en assignant l'implémentation à la propriété <xref:System.ServiceModel.Dispatcher.DispatchRuntime.MessageInspectors%2A>.Pour obtenir des exemples, consultez [Comment : inspecter ou modifier des messages sur le client](../../../../docs/framework/wcf/extending/how-to-inspect-or-modify-messages-on-the-client.md) et [Comment : inspecter ou modifier des messages sur le client](../../../../docs/framework/wcf/extending/how-to-inspect-or-modify-messages-on-the-client.md).  
  
-   Journalisation des messages personnalisés.Un utilisateur souhaite inspecter et enregistrer des jeux de messages d'application qui transitent par un point de terminaison.Cela peut également être accompli à l'aide des interfaces de l'intercepteur de messages.  
  
-   Transformations des messages personnalisés.Plutôt que de modifier le code d'application, l'utilisateur souhaite appliquer certaines transformations au message dans l'exécution \(pour le contrôle de version par exemple\).Cela peut, à nouveau, être accompli à l'aide des interfaces de l'intercepteur de messages.  
  
-   Modèle de données personnalisées.Un utilisateur souhaite disposer d'un modèle de données ou de sérialisation autre que ceux pris en charge par défaut dans [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] \(à savoir, les objets <xref:System.Runtime.Serialization.DataContractSerializer?displayProperty=fullName>, <xref:System.Xml.Serialization.XmlSerializer?displayProperty=fullName> et <xref:System.ServiceModel.Channels.Message?displayProperty=fullName>\).Cela peut être accompli en implémentant les interfaces du formateur de messages.Pour plus d'informations, consultez <xref:System.ServiceModel.Dispatcher.IClientMessageFormatter?displayProperty=fullName> et la propriété <xref:System.ServiceModel.Dispatcher.ClientOperation.Formatter%2A?displayProperty=fullName>.  
  
-   Validation des paramètres personnalisés.Un utilisateur souhaite que les paramètres typés soient valides \(par opposition à XML\).Cela peut être accompli à l'aide des interfaces de l'inspecteur de paramètre.Pour obtenir un exemple, consultez [Comment : inspecter ou modifier des paramètres](../../../../docs/framework/wcf/extending/how-to-inspect-or-modify-parameters.md) ou [Client Validation](../../../../docs/framework/wcf/samples/client-validation.md).  
...
-   <xref:System.ServiceModel.Dispatcher.ClientRuntime.CallbackDispatchRuntime%2A>  
 ...
 Pour obtenir des exemples présentant l'interception sur un client [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)], consultez [Comment : inspecter ou modifier des messages sur le client](../../../../docs/framework/wcf/extending/how-to-inspect-or-modify-messages-on-the-client.md).  
...
### Utilisation de la classe ClientOperation  
 ...
 La propriété <xref:System.ServiceModel.Dispatcher.ClientRuntime.Operations%2A> permet de localiser l'objet <xref:System.ServiceModel.Dispatcher.ClientOperation> qui représente une opération de service spécifique.Les propriétés suivantes vous permettent d'insérer des objets personnalisés dans le système client [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] :  
...
-   La propriété <xref:System.ServiceModel.Dispatcher.ClientOperation.SyncMethod%2A> permet de contrôler la méthode mappée à l'opération.  
 ...
 Pour obtenir des exemples présentant l'interception sur un client [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)], consultez [Comment : inspecter ou modifier des paramètres](../../../../docs/framework/wcf/extending/how-to-inspect-or-modify-parameters.md).  
  
## Voir aussi  
 <xref:System.ServiceModel.Dispatcher.ClientRuntime>   
 <xref:System.ServiceModel.Dispatcher.ClientOperation>   
 [Comment : inspecter ou modifier des messages sur le client](../../../../docs/framework/wcf/extending/how-to-inspect-or-modify-messages-on-the-client.md)   
