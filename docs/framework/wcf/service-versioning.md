...
-   Modifications d'implémentation : par exemple, lorsqu'une implémentation de méthode interne change.  
 ...
 Certaines de ces modifications sont dites « avec rupture » et d'autres « sans rupture ». Une modification est *sans rupture* si tous les messages qui auraient été traités avec succès dans la version précédente sont traités avec succès dans la nouvelle version. Toute modification ne répondant pas à ce critère est une *avec rupture* modifier.  
...
### <a name="lax-versioning"></a>Contrôle de version souple  
 ...
 [!INCLUDE[indigo2](../../../includes/indigo2-md.md)], Les Services Web ASP.NET et nombreuses autres Web service Piles prennent en charge *contrôle de version souple*: autrement dit, ils ne lèvent pas les exceptions pour les nouveaux membres de données inconnu dans les données reçues.  
 ...
 Pour obtenir des instructions détaillées pour le contrôle de version à la fois souple et strict de contrats de données, consultez [meilleures pratiques : contrôle de version de contrat de données](../../../docs/framework/wcf/best-practices-data-contract-versioning.md).  
