---
title: "S&#233;rialisation et d&#233;s&#233;rialisation | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
> [!WARNING]
>  <xref:System.Runtime.Serialization.DataContractSerializer> sérialise les chaînes contenant des caractères de contrôle avec une valeur hexadécimale inférieure à 20 comme entités XML. Cela peut poser un problème avec un client autre que WCF lors de l'envoi de ces données à un service WCF.  
...
### Spécification de l'espace de nom et du nom racine par défaut  
...
#### Activation et désactivation de allers\-retours  
...
### Conservation de graphique d'objet  
...
### Sérialisation simple  
 ...
 Lors de l'utilisation de la méthode <xref:System.Runtime.Serialization.XmlObjectSerializer.WriteObject%2A>, le sérialiseur utilise le nom et l'espace de noms par défaut pour l'élément wrapper et l'écrit avec le contenu \(consultez la section précédente « Spécification de l'espace de nom et du nom racine par défaut »\).  
...
## Désérialisation  
 ...
 Lors de l'utilisation de l'une des surcharges `ReadObject` simples, le désérialiseur recherche le nom et l'espace de noms par défaut sur l'élément wrapper \(consultez la section précédente « Spécification de l'espace de nom et du nom racine par défaut »\) et lève une exception s'il trouve un élément inconnu. Dans l'exemple précédent, l'élément wrapper `<Person>` est attendu. La méthode <xref:System.Runtime.Serialization.XmlObjectSerializer.IsStartObject%2A> est appelée pour vérifier que le lecteur est positionné sur un élément dont le nom est attendu.  
...
-   Les méthodes <xref:System.Runtime.Serialization.NetDataContractSerializer.Serialize%2A> et <xref:System.Runtime.Serialization.NetDataContractSerializer.Deserialize%2A> sont des alias pour les méthodes <xref:System.Runtime.Serialization.XmlObjectSerializer.WriteObject%2A> et <xref:System.Runtime.Serialization.XmlObjectSerializer.ReadObject%2A>. Elles existent pour fournir un modèle de programmation plus cohérent avec la sérialisation binaire ou SOAP.  
 ...
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)] ces fonctionnalités, consultez [Sérialisation binaire](../../../../docs/framework/serialization/binary-serialization.md).  
