---
title: "Protocole de messagerie fiable version&#160;1.0 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   B1102 : lors de l'accès au message `CreateSequence`, le `Responder`[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] envoie et reçoit les deux éléments `Expires` s'ils existent, mais n'utilise pas leurs valeurs.  
...
-   B1201 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère et accède aux numéros de séquence qui ne dépassent pas la valeur maximale de `xs:long`, à savoir 9223372036854775807.  
  
-   B1202 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère toujours un dernier message vide avec l'URI action de http:\/\/schemas.xmlsoap.org\/ws\/2005\/02\/rm\/LastMessage.  
...
-   B1502 :le point de terminaison [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] peut générer des erreurs `CreateSequenceRefused` comme décrit dans la spécification.  
  
-   B1503 :lorsque le point de terminaison du service atteint sa limite de connexion et ne peut pas traiter de nouvelles connexions, [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère un sous\-code d'erreur `CreateSequenceRefused` supplémentaire, `netrm:ConnectionLimitReached`, comme indiqué dans l'exemple suivant.  
...
-   B1601 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère l'erreur Message Addressing Header Required lorsque l'une des conditions suivantes est vraie :  
...
-   B1602 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère l'erreur Action Not Supported en réponse à un message qui n'a pas d'en\-tête `Sequence` et a un en\-tête `Action` qui n'est pas un reconnu dans la spécification de la messagerie WS\-Reliable.  
  
-   B1603 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère l'erreur Endpoint Unavailable pour indiquer que le point de terminaison ne traite pas la séquence au terme de l'examen des en\-têtes d'adressage du message `CreateSequence`.  
...
-   R2102 :une version unique de WS\-Addressing doit être utilisée dans la totalité d'une séquence de messagerie WS\-Reliable donnée ou dans une paire de séquences réciproques corrélées à l'aide du mécanisme `wsrm:Offer`.  
...
-   R2301 :pour protéger l'intégrité d'une séquence de messagerie WS\-Reliable en plus de l'intégrité et de la confidentialité des messages individuels, [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] requiert l'utilisation de WS\-Secure Conversation.  
...
-   B3002 : [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] prend en charge les propriétés facultatives suivantes de l'assertion de messagerie WS\-Reliable et assure leur contrôle sur l'`ReliableMessagingBindingElement` de [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] :  
...
## Extension de messagerie WS\-Reliable pour le contrôle de flux  
 ...
 Le contrôle de flux est activé en affectant à la propriété `FlowControlEnabled``bool` de `ReliableSessionBindingElement` la valeur `true`.Voici une liste des contraintes qui s'appliquent à [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] :  
...
-   B4004 :le service de messagerie fiable de [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] limite le nombre de messages transmis lorsque la destination de la messagerie fiable ne peut pas recevoir les messages rapidement.La destination de la messagerie fiable met en mémoire tampon les messages et la valeur de l'élément tombe à 0.  
  
-   B4005 : [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère des valeurs entières `netrm:BufferRemaining` dans une plage inclusive de 0 à 4096 et lit des valeurs entières dans une plage inclusive de 0 à la valeur `maxInclusive` 214748364 de `xs:int`.  
