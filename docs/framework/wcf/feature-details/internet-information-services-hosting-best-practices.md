---
title: "Meilleures pratiques pour l&#39;h&#233;bergement dans Internet Information Services | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Optimisation des performances dans les scénarios de couche intermédiaire  
 Pour des performances optimales dans un *scénario de couche intermédiaire* \(un service qui appelle d'autres services en réponse à des messages entrants\), instanciez le client du service [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] au service distant une fois et réutilisez\-le dans plusieurs requêtes entrantes.L'instanciation des clients du service [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] est une opération coûteuse par rapport à un appel de service sur une instance cliente préexistante, et les scénarios de couche intermédiaire offrent des gains de performances distincts en mettant en cache des clients distants sur les demandes.Les clients du service [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] sont thread\-safe ; par conséquent, il n'est pas nécessaire de synchroniser l'accès à un client sur plusieurs threads.  
  
 Les scénarios de couche intermédiaire produisent également des gains de performance en utilisant les API asynchrones générées par l'option `svcutil /a`.L'option `/a` entraîne la génération par [Outil Service Model Metadata Tool \(Svcutil.exe\)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) de méthodes `BeginXXX/EndXXX` pour chaque opération de service, ce qui permet d'effectuer des appels potentiellement longs aux services distants sur les threads d'arrière\-plan.  
...
```  
 ...
 Si `aspNetCompatibilityEnabled` a la valeur `true`, configurez <xref:System.ServiceModel.Activation.ServiceHttpHandlerFactory> comme illustré dans l'extrait de configuration suivant.  
...
## Voir aussi  
 [Service Hosting Samples](http://msdn.microsoft.com/fr-fr/f703a3f6-0fba-418a-a92f-7ce75ccfa47e)   
