---
title: "Meilleures pratiques pour les communications mises en file d&#39;attente | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "meilleures pratiques (WCF), communication en file d'attente"
  - "files d'attente (WCF), meilleures pratiques"
...
## Messagerie mise en file attente efficace et rapide  
 ...
 La sécurité a des conséquences sur la performance.[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][Considérations sur les performances](../../../../docs/framework/wcf/feature-details/performance-considerations.md).  
  
## Messagerie mise en file d'attente fiable de bout en bout  
...
### Utilisation des files d'attente de lettres mortes  
 ...
 [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)] [Utilisation de files d'attente de lettres mortes pour gérer des défaillances de transfert de messages](../../../../docs/framework/wcf/feature-details/using-dead-letter-queues-to-handle-message-transfer-failures.md).  
  
### Utilisation de la gestion des messages incohérents  
 La gestion des messages empoisonnés permet de reprendre le traitement des messages après une défaillance.  
  
 Lorsque vous utilisez la fonctionnalité de gestion des messages incohérents, vérifiez que la propriété <xref:System.ServiceModel.MsmqBindingBase.ReceiveErrorHandling%2A> a la valeur appropriée.Lui affecter la valeur <xref:System.ServiceModel.ReceiveErrorHandling> signifie que les données sont perdues.En revanche, lui affecter la valeur <xref:System.ServiceModel.ReceiveErrorHandling> entraîne la défaillance de l'hôte de service lorsqu'il détecte un message incohérent.Avec MSMQ 3.0, <xref:System.ServiceModel.ReceiveErrorHandling> est la meilleure option pour éviter la perte de données et se débarrasser du message incohérent.Avec MSMQ 4.5, <xref:System.ServiceModel.ReceiveErrorHandling> est l'approche recommandée.<xref:System.ServiceModel.ReceiveErrorHandling> déplace un message incohérent hors de la file d'attente afin que le service puisse continuer à traiter de nouveaux messages.Le service des messages incohérents peut ensuite traiter séparément le message incohérent.  
...
-   Limitation.Pour des performances optimales, limitez le nombre de messages dans le pipeline de répartiteur.Pour obtenir un exemple de la marche à suivre, consultez [Throttling](../../../../docs/framework/wcf/samples/throttling.md)..  
 ...
 [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)] [Traitement par lots des messages dans une transaction](../../../../docs/framework/wcf/feature-details/batching-messages-in-a-transaction.md) et [Différences entre les fonctionnalités de mise en file d'attente dans Windows Vista, Windows Server 2003 et Windows XP](../../../../docs/framework/wcf/feature-details/diff-in-queue-in-vista-server-2003-windows-xp.md).  
  
## Mise en file d'attente avec sémantique d'unité de travail  
 ...
 [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)] [Regroupement de messages mis en file d'attente dans une session](../../../../docs/framework/wcf/feature-details/grouping-queued-messages-in-a-session.md).  
  
## Corrélation des messages demande\-réponse  
...
## Intégration à des applications non WCF  
...
-   `MsmqIntegrationBinding` prend en charge les types de sérialisation les plus courants.Selon le type de sérialisation, le type de corps du message générique, <xref:System.ServiceModel.MsmqIntegration.MsmqMessage%601>, prend des paramètres de type différents.Par exemple, <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat> requiert `MsmqMessage\<byte[]>` et <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat> requiert `MsmqMessage<Stream>`.  
  
-   Avec la sérialisation XML, vous pouvez spécifier le type connu à l'aide de l'attribut `KnownTypes` sur l'élément [\<comportement\>](../../../../docs/framework/configure-apps/file-schema/wcf/behavior-of-servicebehaviors.md) qui est ensuite utilisé pour déterminer comment désérialiser le message XML.  
