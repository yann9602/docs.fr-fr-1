---
title: "Protocole de messagerie fiable version 1.1 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   B1101 : l'initiateur [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] utilise la même référence de point de terminaison que les `ReplyTo`, `AcksTo` et `Offer/Endpoint` du message `CreateSequence`.  
...
### Séquences  
 Les éléments suivants sont une liste des contraintes qui s'appliquent aux séquences :  
  
-   B1401 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère et accède aux numéros de séquence qui ne dépassent pas la valeur maximale de `xs:long`, à savoir 9223372036854775807.  
...
-   R1601 : lorsque deux séquences réciproques sont établies à l'aide du mécanisme `Offer`, l'en\-tête `SequenceAcknowledgement` peut être inclus dans tout message d'application transmis au destinataire souhaité.Le point de terminaison distant doit être en mesure d'accéder à un en\-tête `SequenceAcknowledgement` superposé.  
...
-   B1701 : [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] ne génère pas d'erreurs `MessageNumberRollover`.  
...
-   B1801 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère et transmet l'erreur `Message Addressing Header Required` lorsque l'une des conditions suivantes est satisfaite :  
  
    -   Il manque un en\-tête `MessageId` à un message `CreateSequence`, `CloseSequence` ou `TerminateSequence`.  
  
    -   Il manque un en\-tête `ReplyTo` à un message `CreateSequence`, `CloseSequence` ou `TerminateSequence`.  
  
    -   Il manque un en\-tête `RelatesTo` à un message `CreateSequenceResponse`, `CloseSequenceResponse` ou `TerminateSequenceResponse`.  
  
-   B1802 :[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère et transmet l'erreur `Endpoint Unavailable` pour indiquer qu'il n'y a aucun point de terminaison en cours d'écoute pour traiter la séquence en fonction de l'analyse des en\-têtes d'adressage dans le message `CreateSequence`.  
  
## Composition de protocole  
...
-   B3007 : [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] génère et lit les propriétés suivantes de l'assertion WS\-ReliableMessaging et permet de les contrôler sur [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]`ReliableSessionBindingElement` :  
...
## Extension de messagerie WS\-Reliable pour le contrôle de flux  
 ...
 Le contrôle de flux est activé en affectant à la propriété `FlowControlEnabled``boolean` de `ReliableSessionBindingElement` la valeur `true`.Voici une liste des contraintes qui s'appliquent à [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] :  
...
-   B4004 :lorsque le contrôle de flux de messagerie fiable est activé, la source de messagerie fiable [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] utilise la valeur de `netrm:BufferRemaining` pour accélérer la transmission de message.  
