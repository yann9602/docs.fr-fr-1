---
title: "Configuration de HTTP et HTTPS | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
Les services et les clients WCF peuvent communiquer sur HTTP et HTTPS.  Les paramètres HTTP\/HTTPS sont configurés à l'aide d'IIS \(Internet Information Services\) ou d'un outil de ligne de commande.  Lorsqu'un service WCF est hébergé sous IIS, des paramètres HTTP ou HTTPS peuvent être configurés dans IIS \(avec l'outil inetmgr.exe\).  Si un service WCF est auto\-hébergé, les paramètres HTTP ou HTTPS sont configurés à l'aide d'un outil de ligne de commande.  
 ...
 Si vous exécutez [!INCLUDE[ws2003](../../../../includes/ws2003-md.md)] ou [!INCLUDE[wxp](../../../../includes/wxp-md.md)], utilisez l'outil HttpCfg.exe.  [!INCLUDE[ws2003](../../../../includes/ws2003-md.md)] installe automatiquement cet outil.  Si vous exécutez [!INCLUDE[wxp](../../../../includes/wxp-md.md)], vous pouvez télécharger l'outil sur la page [Outils de support Windows XP Service Pack 2](http://go.microsoft.com/fwlink/?LinkId=88606).  [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)] [Vue d'ensemble de Httpcfg](http://go.microsoft.com/fwlink/?LinkId=88605).  
...
### Exécution de Windows XP ou Server 2003  
 Utilisez la commande `httpcfg.exe set urlacl` pour modifier des réservations d'espace de noms.  La [documentation Outils de prise en charge Windows](http://go.microsoft.com/fwlink/?LinkId=94840) explique la syntaxe de l'outil Httpcfg.exe.  La modification des droits de réservation pour une partie de l'espace de noms requiert des privilèges d'administrateur ou la propriété de cette partie de l'espace de noms.  Initialement, la totalité de l'espace de noms HTTP appartient à l'administrateur local.  
...
### Exécution de Windows Vista, Windows Server 2008 R2 ou Windows 7  
...
```  
 ...
 Cette commande ajoute une réservation d'URL pour l'espace de nom d'URL spécifié pour le compte DOMAIN\/utilisateur.  Pour plus d'informations sur l'utilisation de la commande netsh, entrez « netsh http add urlacl » dans une invite de commande et appuyez sur Entrée.  
...
## Configuration de certificats SSL  
 ...
 Pour des instructions pas à pas, consultez [Comment : configurer un port avec un certificat SSL](../../../../docs/framework/wcf/feature-details/how-to-configure-a-port-with-an-ssl-certificate.md).  
...
### Exécution de Windows XP ou Server 2003  
 Utilisez l'outil httpcfg pour modifier la liste d'écoute IP, comme illustré dans l'exemple suivant.  La [documentation Outils de prise en charge Windows](http://go.microsoft.com/fwlink/?LinkId=94840) explique la syntaxe de l'outil httpcfg.exe.  
  
```  
httpcfg.exe set iplisten -i 0.0.0.0:8000  
```  
  
### Exécution de Windows Vista ou Windows 7  
...
## Autres paramètres de configuration  
 ...
 L'API du serveur HTTP inclut un certain nombre de paramètres de configuration avancés qui ne sont pas disponibles via HttpCfg.  Ces paramètres sont maintenus dans le registre et s'appliquent à toutes les applications exécutées sur les systèmes utilisant les API de serveur HTTP.  Pour plus d'informations sur ces paramètres, consultez [Paramètres de registre de Http.sys pour IIS](http://go.microsoft.com/fwlink/?LinkId=94843).  La plupart des utilisateurs n'ont pas besoin de modifier ces paramètres.  
  
## Problèmes spécifiques à Windows XP  
