---
title: "Utilisation de contrats de message | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Définition de contrats de message  
...
## Contrôle des espaces de noms et noms des en\-têtes et parties du corps  
 ...
 Vous pouvez modifier ces valeurs par défaut en manipulant <xref:System.ServiceModel.MessageContractMemberAttribute.Name%2A?displayProperty=fullName> et <xref:System.ServiceModel.MessageContractMemberAttribute.Namespace%2A?displayProperty=fullName> \(sur la classe parente des attributs <xref:System.ServiceModel.MessageHeaderAttribute> et <xref:System.ServiceModel.MessageBodyMemberAttribute>\).  
...
## Contrôle de l'encapsulation des parties de corps SOAP  
 ...
 Pour supprimer l'élément wrapper, affectez `false` à la propriété <xref:System.ServiceModel.MessageContractAttribute.IsWrapped%2A>.Pour contrôler le nom et l'espace de noms de l'élément wrapper, utilisez les propriétés <xref:System.ServiceModel.MessageContractAttribute.WrapperName%2A> et <xref:System.ServiceModel.MessageContractAttribute.WrapperNamespace%2A>.  
...
## Utilisation de tableaux dans des contrats de message  
...
## Utilisation de tableaux d'octets dans des contrats de message  
...
## Signature et chiffrement des parties du message  
 ...
 Pour ce faire, définissez la propriété <xref:System.ServiceModel.MessageContractMemberAttribute.ProtectionLevel%2A?displayProperty=fullName> sur les attributs <xref:System.ServiceModel.MessageHeaderAttribute> et <xref:System.ServiceModel.MessageBodyMemberAttribute>.La propriété est une énumération du type <xref:System.Net.Security.ProtectionLevel?displayProperty=fullName> et peut être définie à <xref:System.Net.Security.ProtectionLevel> \(pas de chiffrement ni de signature\), <xref:System.Net.Security.ProtectionLevel> \(signature numérique uniquement\) ou <xref:System.Net.Security.ProtectionLevel> \(chiffrement et signature numérique\).La valeur par défaut est <xref:System.Net.Security.ProtectionLevel>.  
...
```  
 ...
 Dans cet exemple, l'en\-tête `recordID` n'est pas protégé, `patientName` est `signed`, et `SSN` est chiffré et signé.<xref:System.Net.Security.ProtectionLevel> étant appliqué sur au moins une partie du corps \(`medicalHistory`\), l'ensemble du message est donc chiffré et signé, même si les commentaires et les parties de corps de diagnostic spécifient des niveaux de protection inférieurs.  
  
## Action SOAP  
 SOAP et les standards de services Web associés définissent une propriété appelée `Action` qui peut être présente pour chaque message SOAP envoyé.Les propriétés <xref:System.ServiceModel.OperationContractAttribute.Action%2A?displayProperty=fullName> et <xref:System.ServiceModel.OperationContractAttribute.ReplyAction%2A?displayProperty=fullName> de l'opération contrôlent la valeur de cette propriété.  
...
-   `Relay`  
 ...
 Lors de l'envoi d'un message, ces attributs ne sont pas émis par défaut.Vous pouvez modifier ce paramètre de deux façons :La première méthode consiste à affecter statiquement aux attributs les valeurs souhaitées en modifiant les propriétés <xref:System.ServiceModel.MessageHeaderAttribute.Actor%2A?displayProperty=fullName>, <xref:System.ServiceModel.MessageHeaderAttribute.MustUnderstand%2A?displayProperty=fullName> et <xref:System.ServiceModel.MessageHeaderAttribute.Relay%2A?displayProperty=fullName>, tel qu'indiqué dans l'exemple de code suivant.\(Notez qu'il n'y a pas de propriété `Role` ; la définition de la propriété <xref:System.ServiceModel.MessageHeaderAttribute.Actor%2A> entraîne l'émission de l'attribut `Role` si vous utilisez SOAP 1.2\).  
...
## Ordre des parties de corps SOAP  
 Dans certains cas, il peut s'avérer nécessaire de contrôler l'ordre des parties du corps.L'ordre des éléments du corps est alphabétique par défaut, mais peut être contrôlé par la propriété <xref:System.ServiceModel.MessageBodyMemberAttribute.Order%2A?displayProperty=fullName>.Cette propriété a la même sémantique que la propriété <xref:System.Runtime.Serialization.DataMemberAttribute.Order%2A?displayProperty=fullName>, à l'exception du comportement dans les scénarios d'héritage \(dans les contrats de message, les membres du corps du type de base ne sont pas triés avant les membres du corps du type dérivé\).[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][Classement des membres de données](../../../../docs/framework/wcf/feature-details/data-member-order.md).  
...
## Considérations sur WSDL  
 Lors de la génération d'un contrat WSDL \(Web Services Description Language\) à partir d'un service qui utilise des contrats de message, il est important de se souvenir que les fonctionnalités de contrat de message ne sont pas toutes répercutées dans le WSDL résultant.Examinons les points suivants :  
...
## Considérations sur l'encodage SOAP  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] vous permet d'utiliser le style d'encodage SOAP hérité de XML, bien que cela ne soit pas recommandé.Lors de l'utilisation de ce style \(en affectant à la propriété `Use` la valeur `Encoded` sur le <xref:System.ServiceModel.XmlSerializerFormatAttribute?displayProperty=fullName> appliqué au contrat de service\), les considérations supplémentaires suivantes s'appliquent :  
...
-   L'encodage SOAP conserve les références d'objet.Examinons par exemple le code suivant.  
...
### Contrats de message et modèle asynchrone basé sur les événements  
