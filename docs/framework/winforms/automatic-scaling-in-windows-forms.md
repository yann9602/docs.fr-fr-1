---
title: "Mise &#224; l&#39;&#233;chelle automatique dans les Windows Forms | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  - "jsharp"
helpviewer_keywords: 
  - "extensibilité, automatique dans (Windows Forms)"
  - "Windows Forms, mise à l'échelle automatique"
...
## Besoin de mise à l'échelle automatique  
 ...
 Une situation analogue se produit quand une application est conçue pour une certaine résolution d'écran.  La résolution d'affichage la plus courante est de 96 points par pouce \(PPP\), mais les résolutions plus élevée prenant en charge 120, 133, 170  pouces et plus sont de plus en plus courantes.  Sans réglage, une application \(en particulier basée sur des graphismes\) conçue pour une résolution spécifique apparaîtra trop grande ou trop petite en cas d'exécution dans une autre résolution.  
 ...
 [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] prend en charge la mise à l'échelle automatique en fonction de la police système ou de la résolution d'affichage par défaut.  Le cas échéant, la mise à l'échelle automatique peut être désactivée dans une application.  
  
## Prise en charge d'origine de la mise à l'échelle automatique  
 Les versions 1.0 et 1.1 de [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] prenaient en charge la mise à l'échelle automatique d'une manière simple qui dépendait de la police Windows par défaut utilisée pour l'interface utilisateur, représentée par la valeur **DEFAULT\_GUI\_FONT** du SDK Win32.  Cette police est généralement modifiée uniquement quand la résolution d'écran change.  Le mécanisme suivant était utilisé pour implémenter la mise à l'échelle automatique :  
...
-   Étant donné que la propriété <xref:System.Windows.Forms.Form.AutoScaleBaseSize%2A> représente la taille de police de base sous forme de valeurs intégrales, des erreurs d'arrondi apparaissent clairement quand un formulaire passe par plusieurs résolutions.  
...
4.  <xref:System.Windows.Forms.ContainerControl.PerformAutoScale%2A> est également appelée automatiquement dans les situations suivantes :  
    ...
    -   En réponse à l'événement <xref:System.Windows.Forms.Control.OnFontChanged%2A> si le mode de mise à l'échelle est <xref:System.Windows.Forms.AutoScaleMode>.  
...
## Voir aussi  
 ...
 [Comment : améliorer les performances en évitant la mise à l'échelle automatique](../../../docs/framework/winforms/advanced/how-to-improve-performance-by-avoiding-automatic-scaling.md)