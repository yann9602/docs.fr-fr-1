...
2.  Configurez votre commutateur de trace. Pour plus d'informations, voir [Configuration des commutateurs de trace](#configure).  
...
<a name="configure"></a>   
## Configurer les commutateurs de trace  
 ...
 Vos commutateurs sont configurés à l'aide du fichier .config. Pour une application web, il s'agit du fichier Web.config associé au projet. Dans une application Windows, ce fichier est nommé « \(nom\_application\).exe.config ». Dans une application déployée, ce fichier doit résider dans le même dossier que le fichier exécutable.  
...
1.  Pour utiliser les commutateurs de trace, vous devez d’abord les créer et les placer dans votre code, comme décrit dans la section [Pour créer et initialiser un commutateur de trace](#create).  
...
2.  Si votre projet ne contient pas de fichier de configuration \(app.config ou Web.config\), dans le menu **Projet**, sélectionnez **Ajouter un nouvel élément**.  
    ...
    -   **Visual C\#** : dans la boîte de dialogue **Ajouter un nouvel élément**, choisissez **Fichier XML**. Nommez ce fichier **app.config**. Dans l'éditeur XML, après la déclaration XML, ajoutez les balises suivantes :  
        ...
        ```  
         ...
         Au moment de la compilation de votre projet, le fichier app.config est copié dans le dossier de sortie du projet et renommé « *nom\_application*.exe.config ».  
