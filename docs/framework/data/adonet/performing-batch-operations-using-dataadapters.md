---
title: "Ex&#233;cution de traitements par lots &#224; l&#39;aide de DataAdapters | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Ex&#233;cution de traitements par lots &#224; l&#39;aide de DataAdapters
...
## Utilisation de la propriété UpdateBatchSize  
 Lorsque les mises à jour par lots sont activées, la valeur <xref:System.Data.UpdateRowSource> ou <xref:System.Data.UpdateRowSource> doit être affectée à la propriété <xref:System.Data.IDbCommand.UpdatedRowSource%2A> de `UpdateCommand`, `InsertCommand` et `DeleteCommand` du DataAdapter.  Lors de l'exécution d'une mise à jour par lots, les valeurs <xref:System.Data.UpdateRowSource> ou <xref:System.Data.UpdateRowSource> de la propriété <xref:System.Data.IDbCommand.UpdatedRowSource%2A> de la commande ne sont pas valides.  
  
 La procédure suivante illustre l'utilisation de la propriété `UpdateBatchSize`.  La procédure prend deux arguments, un objet <xref:System.Data.DataSet> contenant des colonnes représentant les champs **ProductCategoryID** et **Name** dans la table **Production.ProductCategory**, et un nombre entier représentant la taille de lot \(nombre de lignes dans le lot\).  Le code crée un nouvel objet <xref:System.Data.SqlClient.SqlDataAdapter>, en définissant ses propriétés <xref:System.Data.SqlClient.SqlDataAdapter.UpdateCommand%2A>, <xref:System.Data.SqlClient.SqlDataAdapter.InsertCommand%2A> et <xref:System.Data.SqlClient.SqlDataAdapter.DeleteCommand%2A>.  Le code est basé sur l'hypothèse que l'objet <xref:System.Data.DataSet> comporte des lignes modifiées.  Il définit la propriété `UpdateBatchSize` et effectue la mise à jour.  
...
## Gestion des événements et des erreurs liés aux mises à jour par lots.  
 Le **DataAdapter** comporte deux événements liés aux mises à jour : **RowUpdating** et **RowUpdated**.  Dans les versions précédentes d'ADO.NET, en cas de désactivation du traitement par lots, chacun de ces événements était généré une fois pour chaque ligne traitée.  **RowUpdating** est généré avant la mise à jour et **RowUpdated** après la mise à jour de la base de données.  
