...
|**PreserveCurrentValues (default)**|Si des lignes entrantes ont la même **PrimaryKey** valeur comme une ligne figurant déjà dans le **DataTable**, le **d’origine** valeur est définie sur le contenu de la ligne entrante et la **Actuel** valeur n’est pas modifiée.<br /><br /> Si le **RowState** est **Added** ou **modifié**, elle est définie sur **modifié**.<br /><br /> Si le **RowState** a été **Deleted**, il reste **Deleted**.<br /><br /> Lignes de la source de données qui n’existent pas déjà dans le **DataTable** sont ajoutés et le **RowState** a la valeur **Unchanged**.|  
