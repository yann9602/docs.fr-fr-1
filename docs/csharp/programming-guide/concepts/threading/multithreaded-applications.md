---
title: Applications multithread (C#) | Microsoft Docs
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
## <a name="creating-and-using-threads"></a>Création et utilisation des threads  
 ...
 Vous créez un thread en déclarant une variable de type <xref:System.Threading.Thread> et en appelant le constructeur, en fournissant le nom de la procédure ou de la méthode que vous voulez exécuter sur le nouveau thread. Le code suivant fournit un exemple.  
...
### <a name="starting-and-stopping-threads"></a>Démarrage et arrêt des threads  
 Pour démarrer l’exécution d’un nouveau thread, utilisez la méthode <xref:System.Threading.Thread.Start%2A>, comme illustré dans le code suivant.  
  
```csharp  
newThread.Start();  
```  
  
 Pour arrêter l’exécution d’un thread, utilisez la méthode <xref:System.Threading.Thread.Abort%2A>, comme illustré dans le code suivant.  
  
```csharp  
newThread.Abort();  
```  
  
 En plus de démarrer et d’arrêter des threads, vous pouvez également les suspendre en appelant la méthode <xref:System.Threading.Thread.Sleep%2A> ou <xref:System.Threading.Thread.Suspend%2A>, reprendre un thread suspendu en utilisant la méthode <xref:System.Threading.Thread.Resume%2A> et détruire un thread en utilisant la méthode <xref:System.Threading.Thread.Abort%2A>.  
...
## <a name="thread-priorities"></a>Priorités des threads  
 Chaque thread a une propriété priority, qui détermine la taille d’un intervalle de temps processeur pendant lequel il doit s’exécuter. Le système d’exploitation alloue des intervalles de temps plus longs aux threads à priorité élevée, et des intervalles de temps plus courts pour les threads de priorité basse. Les nouveaux threads sont créés avec la valeur `Normal`, mais vous pouvez modifier la propriété <xref:System.Threading.Thread.Priority%2A> et lui donner une des valeurs de l’énumération <xref:System.Threading.ThreadPriority>.  
  
 Consultez <xref:System.Threading.ThreadPriority> pour obtenir une description détaillée des différentes priorités des threads.  
  
## <a name="foreground-and-background-threads"></a>Threads de premier plan et d'arrière-plan  
 Un *thread de premier plan* peut s’exécuter indéfiniment, tandis qu’un *thread d’arrière-plan* s’arrête dès que le dernier thread de premier plan s’est arrêté. Vous pouvez utiliser la propriété <xref:System.Threading.Thread.IsBackground%2A> pour déterminer ou changer l’état d’arrière-plan d’un thread.  
