...
## <a name="method-signatures"></a>Signatures de méthode  
 Les méthodes sont déclarées dans une [classe](../../../csharp/language-reference/keywords/class.md) ou un [struct](../../../csharp/language-reference/keywords/struct.md) en spécifiant le niveau d’accès, comme `public` ou `private`, des modificateurs facultatifs, comme `abstract` ou `sealed`, la valeur de retour, le nom de la méthode et les éventuels paramètres de la méthode. Ces parties forment ensemble la signature de la méthode.  
