---
title: Erreur du compilateur CS0246 | Microsoft Docs
ms.date: 2015-07-20
ms.prod: .net
ms.technology:
- devlang-csharp
...
-   Si l’erreur concerne un nom d’espace de noms, vous avez peut-être ajouté une référence ([/reference](../../../csharp/language-reference/compiler-options/reference-compiler-option.md)) à l’assembly qui contient l’espace de noms. Par exemple, votre code peut contenir la directive `using Accessibility`. Toutefois, si votre projet ne référence pas l’assembly Accessibility.dll, l’erreur CS0246 se produit. Pour plus d’informations, consultez [Gestion des références dans un projet](https://docs.microsoft.com/visualstudio/ide/managing-references-in-a-project).  
