# [Mots clés C#](index.md)
## [Types](types.md)
### [Types valeur](value-types.md)
#### [bool](bool.md)
#### [byte](byte.md)
#### [char](char.md)
#### [decimal](decimal.md)
#### [double](double.md)
#### [enum](enum.md)
#### [float](float.md)
#### [int](int.md)
#### [long](long.md)
#### [sbyte](sbyte.md)
#### [short](short.md)
#### [struct](struct.md)
#### [uint](uint.md)
#### [ulong](ulong.md)
#### [ushort](ushort.md)
### [Types référence](reference-types.md)
#### [class](class.md)
#### [delegate](delegate.md)
#### [dynamic](dynamic.md)
#### [interface](interface.md)
#### [object](object.md)
#### [string](string.md)
#### [Chaînes interpolées (Référence C# et Visual Basic)](interpolated-strings.md)
### [void](void.md)
### [var](var.md)
### [Tableaux de référence des types](reference-tables-for-types.md)
#### [Tableau des types intégrés](built-in-types-table.md)
#### [Tableau des types intégraux](integral-types-table.md)
#### [Tableau des types virgule flottante](floating-point-types-table.md)
#### [Tableau des valeurs par défaut](default-values-table.md)
#### [Tableau des types valeur](value-types-table.md)
#### [Tableau des conversions numériques implicites](implicit-numeric-conversions-table.md)
#### [Tableau des conversions numériques explicites](explicit-numeric-conversions-table.md)
#### [Tableau des formats des résultats numériques](formatting-numeric-results-table.md)
## [Modificateurs](modifiers.md)
### [Modificateurs d’accès](access-modifiers.md)
#### [Niveaux d’accessibilité](accessibility-levels.md)
#### [Domaine d’accessibilité](accessibility-domain.md)
#### [Limitations sur l’utilisation des niveaux d’accessibilité](restrictions-on-using-accessibility-levels.md)
#### [internal](internal.md)
#### [private](private.md)
#### [protected](protected.md)
#### [public](public.md)
### [abstract](abstract.md)
### [async](async.md)
### [const](const.md)
### [event](event.md)
### [extern](extern.md)
### [in (modificateur générique)](in-generic-modifier.md)
### [out (modificateur générique)](out-generic-modifier.md)
### [override](override.md)
### [readonly](readonly.md)
### [sealed](sealed.md)
### [static](static.md)
### [unsafe](unsafe.md)
### [virtual](virtual.md)
### [volatile](volatile.md)
## [Mots clés d’instructions](statement-keywords.md)
### [Instructions de sélection](selection-statements.md)
#### [if-else](if-else.md)
#### [switch](switch.md)
### [Instructions d’itération](iteration-statements.md)
#### [do](do.md)
#### [for](for.md)
#### [foreach, in](foreach-in.md)
#### [while](while.md)
### [Instructions de saut](jump-statements.md)
#### [break](break.md)
#### [continue](continue.md)
#### [goto](goto.md)
#### [return](return.md)
### [Instructions de gestion des exceptions](exception-handling-statements.md)
#### [throw](throw.md)
#### [try-catch](try-catch.md)
#### [try-finally](try-finally.md)
#### [try-catch-finally](try-catch-finally.md)
### [Checked et unchecked](checked-and-unchecked.md)
#### [checked](checked.md)
#### [unchecked](unchecked.md)
### [fixed, instruction](fixed-statement.md)
### [lock, instruction](lock-statement.md)
## [Paramètres de méthodes](method-parameters.md)
### [params](params.md)
### [ref](ref.md)
### [out](out.md)
#### [out (modificateur de paramètre)](out-parameter-modifier.md)
## [Mots clés d’espaces de noms](namespace-keywords.md)
### [namespace](namespace.md)
### [using](using.md)
#### [using, directive](using-directive.md)
#### [using static, directive](using-static.md)
#### [using, instruction](using-statement.md)
### [alias extern](extern-alias.md)
## [Mots clés des opérateurs](operator-keywords.md)
### [as](as.md)
### [await](await.md)
### [is](is.md)
### [new](new.md)
#### [new, opérateur](new-operator.md)
#### [new, modificateur](new-modifier.md)
#### [new, contrainte](new-constraint.md)
### [sizeof](sizeof.md)
### [typeof](typeof.md)
### [true](true.md)
#### [true, opérateur](true-operator.md)
#### [true, littéral](true-literal.md)
### [false](false.md)
#### [false, opérateur](false-operator.md)
#### [false, littéral](false-literal.md)
### [stackalloc](stackalloc.md)
### [nameof (Référence C# et Visual Basic)](nameof.md)
## [Mots clés de conversion](conversion-keywords.md)
### [explicit](explicit.md)
### [implicit](implicit.md)
### [operator](operator.md)
## [Mots clés d’accès](access-keywords.md)
### [base](base.md)
### [this](this.md)
## [Mots clés littéraux](literal-keywords.md)
### [null](null.md)
### [default](default.md)
## [Mots clés contextuels](contextual-keywords.md)
### [add](add.md)
### [get](get.md)
### [global](global.md)
### [partial (type)](partial-type.md)
### [partial (méthode)](partial-method.md)
### [remove](remove.md)
### [set](set.md)
### [when (condition du filtre)](when.md)
### [where (contrainte de type générique)](where-generic-type-constraint.md)
### [value](value.md)
### [yield](yield.md)
## [Mots clés de requête](query-keywords.md)
### [from, clause](from-clause.md)
### [where, clause](where-clause.md)
### [select, clause](select-clause.md)
### [group, clause](group-clause.md)
### [into](into.md)
### [orderby, clause](orderby-clause.md)
### [join, clause](join-clause.md)
### [let, clause](let-clause.md)
### [ascending](ascending.md)
### [descending](descending.md)
### [on](on.md)
### [equals](equals.md)
### [by](by.md)
### [in](in.md)

