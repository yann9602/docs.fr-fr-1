...
## <a name="remarks"></a>Notes  
 ...
 Le bloc `try` contient le code protégé susceptible de provoquer l'exception. Le bloc est exécuté jusqu'à ce qu'une exception soit levée ou qu'il se soit correctement terminé. Par exemple, la tentative suivante d’effectuer un cast un `null` objet déclenche le <xref:System.NullReferenceException> exception :  
...
```  
 ...
 A [lever](../../../csharp/language-reference/keywords/throw.md) instruction peut être utilisée dans un `catch` bloc à lever de nouveau l’exception est interceptée par le `catch` instruction. L’exemple suivant extrait des informations de source d’un <xref:System.IO.IOException> exception, puis lève l’exception à la méthode parente.  
...
```  
 ...
 Pour plus d’informations sur catch, consultez [try-catch-finally](../../../csharp/language-reference/keywords/try-catch-finally.md).  
  
## <a name="exceptions-in-async-methods"></a>Exceptions dans les méthodes async  
 Une méthode async est marquée par un [async](../../../csharp/language-reference/keywords/async.md) modificateur et contient généralement un ou plusieurs await expressions ou instructions. Applique une expression await la [await](../../../csharp/language-reference/keywords/await.md) opérateur à un <xref:System.Threading.Tasks.Task> ou <xref:System.Threading.Tasks.Task%601>.  
...
## <a name="example"></a>Exemple  
 L’exemple suivant illustre la gestion des exceptions quand plusieurs tâches peuvent entraîner plusieurs exceptions. Le `try` bloc attend la tâche retournée par un appel à <xref:System.Threading.Tasks.Task.WhenAll%2A?displayProperty=fullName>. La tâche est terminée quand les trois tâches auxquelles WhenAll est appliqué sont terminées.  
  
 Chacune de ces trois tâches provoque une exception. Le `catch` bloc effectue une itération dans les exceptions qui sont trouvent dans le `Exception.InnerExceptions` propriétés de la tâche qui a été renvoyée par <xref:System.Threading.Tasks.Task.WhenAll%2A?displayProperty=fullName>.  
...
## <a name="see-also"></a>Voir aussi  
 ...
 [try, throw et catch instructions (C++)](/visual-cpp/cpp/try-throw-and-catch-statements-cpp)   
